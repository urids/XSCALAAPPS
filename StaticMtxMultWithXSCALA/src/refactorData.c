...
void refactorData(){
	...
	int numBlocks=4;
	 rowsAs = sqrt(numBlocks); /* number of rows in _partition_ */
...
/*	int tmpTypeSize,endTypeSize;
	MPI_Type_size(typeVectmp_A, &tmpTypeSize);
	MPI_Type_size(typeVec_A, &endTypeSize);
	printf("sizeoftmp=%d- sizeofEnd %d \n",tmpTypeSize,endTypeSize);*/
	...
	int myRow;
	int myCol;
	int *PA = malloc(sizeof(int)*numBlocks);
	int *PB = malloc(sizeof(int)*numBlocks);

	 myRight = malloc(sizeof(int)*numBlocks);
	 myDown = malloc(sizeof(int)*numBlocks);

	for(i=0;i<numBlocks;i++){
		myRow = floor(i/colsAs);
		myCol = i-(myRow*colsAs);
		PA[i]=((i + myRow) % colsAs)+(myRow*colsAs);
		PB[i]=(i +(myCol*colsAs))%numBlocks;

		myRight[i]=((i+1)%colsAs)+(myRow*colsAs);
		myDown[i]=(i+colsAs)%numBlocks;
	}
