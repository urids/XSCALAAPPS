...
void createData(){
	...
	const char* fileD_Name="dataFile_D.dat";
	...
	MPI_File_open(MPI_COMM_SELF,fileD_Name, MPI_MODE_WRONLY | MPI_MODE_CREATE,MPI_INFO_NULL,&dataFile_D);

	float* A=malloc(rowsA*colsA*sizeof(float));
	float* B=malloc(rowsB*colsB*sizeof(float));
	float* C=malloc(rowsC*colsC*sizeof(float));
	float* D=malloc(rowsD*colsD*sizeof(float));
	...
	MPI_File_write(dataFile_D, D, rowsD*colsD, MPI_FLOAT, MPI_STATUS_IGNORE );
	MPI_File_close(&dataFile_A);
	MPI_File_close(&dataFile_B);
	MPI_File_close(&dataFile_C);
	MPI_File_close(&dataFile_D);
