...
void refactorData(){
	 ...
	 rowsAs = sqrt(numBcks); /* number of rows in _partition_ */
	...
	int myRow;
	int myCol;
	int *PA = malloc(sizeof(int)*numBcks);
	int *PB = malloc(sizeof(int)*numBcks);

	 myRight = malloc(sizeof(int)*numBcks);
	 myDown = malloc(sizeof(int)*numBcks);

	for(i=0;i<numBcks;i++){
		myRow = floor(i/colsAs);
		myCol = i-(myRow*colsAs);
		PA[i]=((i + myRow) % colsAs)+(myRow*colsAs);
		PB[i]=(i +(myCol*colsAs))%numBcks;

		myRight[i]=((i+1)%colsAs)+(myRow*colsAs);
		myDown[i]=(i+colsAs)%numBcks;
	}
